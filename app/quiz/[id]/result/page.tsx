    ...
    const fetchResult = async () => {
      try {
        ...
        if (quizRes.ok) {
          ...
          const participantName = storedResult 
            ? JSON.parse(storedResult).participantName 
            : 'Anda';
        ...
        <div className="grid lg:grid-cols-[320px_1fr] gap-8">
            ...
            <div className="relative w-48 h-48 mx-auto mb-6">
              <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                ...
                <circle
                  ...
                  strokeWidth="8"
                  strokeLinecap="round"
                  strokeDasharray={`${(userScore / 100) * 283} 283`}
                />
              </svg>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
            <h2 className="text-xl font-bold text-gray-800 mb-6">
              Leaderboard ({totalParticipants} siswa)
            </h2>
            ...
            <div className="space-y-3">
              {leaderboard.map((entry) => (
                <div
                  key={entry.rank}
                  className={`flex items-center justify-between p-4 rounded-xl ${getRankBgColor(entry.rank, entry.isCurrentUser)}`}
                >
                  <div className="flex items-center gap-4">
                    <span className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold ${
                      entry.isCurrentUser ? 'bg-cyan-500 text-white' : 'bg-white/50 text-gray-700'
                    }`}>
                      {entry.rank}
                    </span>
                  ...
                  <span className={`font-bold text-lg ${
                    entry.isCurrentUser ? 'text-white' : 'text-gray-800'
                  }`}>
                    {entry.score}
                  </span>
                </div>
              ...
              {userRank > 5 && (
                <>
                  ...
                  <div className="flex items-center justify-between p-4 rounded-xl bg-cyan-400">
                    <div className="flex items-center gap-4">
                      <span className="w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold bg-cyan-500 text-white">
                        {userRank}
                      </span>
                      <div className="w-8 h-8 rounded-full bg-gray-600 flex items-center justify-center overflow-hidden">
                        <span className="text-white text-xs font-medium">
                          {result?.participantName?.substring(0, 2).toUpperCase() || 'AN'}
                        </span>
                      </div>
                      <span className="font-medium text-white">
                        You ({result?.participantName || 'Anda'})
                      </span>
                    </div>
