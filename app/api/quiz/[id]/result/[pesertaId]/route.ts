...
export async function GET(request: NextRequest, context: any) {
  try {
    ...
    const { data: quiz, error: quizError } = await supabase
      .from('kuis')
      .select('kuis_id, judul, durasi_menit, total_soal, tingkat_kesulitan, kkm')
      .eq('kuis_id', quizId)
      .single();
    ...
    const { data: participant, error: participantError } = await supabase
      .from('peserta_kuis')
      .select('peserta_id, nama_siswa, user_id')
    ...
    const questionResults = (questions || []).map((q: any) => {
      ...
      return {
        ...
        jawaban_benar: q.tipe_soal === 'pilihan_ganda' 
          ? correctOption?.teks_pilihan 
          : keyAnswer?.jawaban_text,
