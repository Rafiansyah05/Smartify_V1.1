...
export async function POST(request: NextRequest, context: any) {
  try {
    ...
    const { data: quiz, error: quizError } = await supabase.from('kuis').select('kuis_id, durasi_menit').eq('kuis_id', quizId).single();
    ...
    const { data: participants, error: participantsError } = await supabase.from('peserta_kuis').select('peserta_id, user_id, nama_siswa').eq('kuis_id', quizId);
    ...
    if (participants && participants.length > 0) {
      ...
      const { error: hasilError } = await supabase.from('hasil_kuis').upsert(hasilKuisRecords, { onConflict: 'peserta_id' });
