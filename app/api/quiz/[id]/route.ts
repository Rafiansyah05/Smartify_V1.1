...
export async function GET(request: NextRequest, context: any) {
  try {
    ...
    // Ambil data kuis
    const { data: kuis, error: kuisError } = await supabase.from('kuis').select('*').eq('kuis_id', quizId).single();
    ...
    // Ambil soal
    const { data: soalList, error: soalError } = await supabase.from('soal').select('*').eq('kuis_id', quizId).order('urutan', { ascending: true });
    ...
    const populatedSoal = await Promise.all(
      soalList.map(async (soal) => {
        if (soal.tipe_soal === 'pilihan_ganda') {
          const { data: pilihan } = await supabase.from('pilihan_jawaban').select('*').eq('soal_id', soal.soal_id).order('urutan', { ascending: true });

          const { data: kunci } = await supabase.from('kunci_jawaban').select('*').eq('soal_id', soal.soal_id).single();

          return { ...soal, pilihan: pilihan || [], kunci_jawaban: kunci };
        } else {
          const { data: kunci } = await supabase.from('kunci_jawaban').select('*').eq('soal_id', soal.soal_id).single();
